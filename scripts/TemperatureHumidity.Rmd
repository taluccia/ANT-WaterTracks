...
```{r eval=FALSE, include=FALSE}
gt::gtsave(t4, filename = "../outputs/Tables/HGOTSoilProfile.html")
```
...
theme_reg = theme_bw() + theme(legend.position = "none") +
  ...
  theme(axis.title.x = element_text(size = 9),
        axis.title.y = element_text(size = 9, hjust = 0, vjust = 0),
...
## Define Labels
```{r}
ecUnits = "Electrocal conductivity (mS/cm)" 
vwcUnits = "Moisture (m³/m³)" 
...
( plot1 <- ggplot(NULL, aes(x=date, y=airtemp)) + 
      geom_point(data = surfaceOff2, aes(color="A"), shape=21) +
      geom_line(data = meanOffsurface, aes(color="B"), ) +
  labs(x="Date", y=tempLab, caption = "Off track") +
    scale_color_manual(values = c("A" = "#a1dab4","B"="#253494"), labels =c("A" ="2 minutes", "B"="Mean daily")) +
  theme_reg
  )
```
...
( plot4 <- ggplot() + 
  ...
  labs(x="Date", y=tempLab, caption = "Vertical profile off track") +
     scale_color_manual(values = c("A" = "#016c59","B"="#1c9099", "C"="#67a9cf", "D"="#bdc9e1"), labels =c("A" ="0 cm", "B"="10 cm", "C"="20 cm", "D"="30 cm")) +
  theme_reg
 )
```
...
( plot6 <- ggplot(NULL, aes(x=date, y=airtemp)) + 
      geom_point(data = surfaceOn2, aes(color="A"), shape=21) +
...
( plot9 <- ggplot() + 
  ...
  labs(x="Date", y=tempLab, caption = "Vertical profile on track") +
     scale_color_manual(values = c("A" = "#016c59","B"="#1c9099", "C"="#67a9cf", "D"="#bdc9e1"), labels =c("A" ="0 cm", "B"="10 cm", "C"="20 cm", "D"="30 cm")) +
  theme_reg
 )
```
...
```{r}
( g1 = cowplot::plot_grid(plot1, plot6, plot2,plot7, ncol=2, labels = "auto")
  )
```
...
```{r eval=FALSE, include=FALSE}
ggsave("../figures/SurfaceMeasures.tiff", plot = g1, width = 6, height =5, units = c("in"), dpi=600, bg = "white" )
```
...
```{r eval=FALSE, include=FALSE}
ggsave("../figures/VerticalProfileMeasures.tiff", plot = g2, width = 6, height =7, units = c("in"), dpi=600, bg = "white" )
```